...
const RegisterNewStudent = () => {
  ...
  async function storeStudent() {
    ...
    try {
      const response = await api.post("alunos", studentsData).then((data) => {
        Alert.alert('Aluno Cadastrado', 'Cadastro realizado com sucesso!');
        console.log(data);
        setLoading(false);
        navigation.navigate('StudentRegister');
      }).catch(err => {
        Alert.alert('Falha ao Cadastrar', 'Favor verifique se os dados foram digitados corretamente.');
        console.log(err);
        setLoading(false);
      });
      return response;
  ...
  return (
    <Container>
      ...
      >
        ...
        <FieldsContainer>
          ...
          <LabelAndInputContainer>
            ...
            <FormInputTextMasked
              ...
              onChangeText={(text) => {
                setStudentsData({ ...studentsData, data_conclusao_ensino_medio: 
                  text.split('/').join('-')
                })
              }}
              keyboardType="phone-pad"
          ...
          <LabelAndInputContainer>
            ...
            <FormInputTextMasked
              ...
              onChangeText={(text) => {
                setStudentsData({ ...studentsData, data_expedicao_rg: 
                  text.split('/').join('-')
                })
              }}
              keyboardType="phone-pad"
        ...
        <RegisterButton
          onPress={() => storeStudent()}
          disabled={
            typeof (studentsData.nome) && typeof(studentsData.data_de_nascimento) == 'undefined' ? true : false
          }
          style={{
            opacity: typeof(studentsData.nome) && typeof(studentsData.data_de_nascimento) == 'undefined' ? 0.6 : 1
          }}
        >
          {loading ?
            (
              ...
              <TextButton>Salvar novo Aluno</TextButton>
            )
          }
        </RegisterButton>
